...
def get_location_by_city(city_name):
    geolocator = Nominatim(user_agent="holanoisemmatare")
...
def send_data(ra, dec, date, lat, lon, time, server_ip='192.168.1.128', server_port=65432):
    print(ra, dec, date, lat, lon, time)
    try:
...
def main(page: ft.Page):
    ...
    def intro_nom_view():
        ...
        if (info_s == None):
            if (info_c == None):
                ...
                ra_const = get_ra_const_from_constellation(nom_introduit)
                dec_const = get_dec_const_from_constellation(nom_introduit)
                send_data_constellation(ra_const, dec_const, date_input.value, latitud, longitud, time_input.value)
            ...
            send_data(ra, dec, date_input.value, latitud, longitud, time_input.value)
    ...
    def send_data_constellation(ra, dec, date, lat, lon, time, server_ip='192.168.1.128', server_port=65432):
        print(ra, dec, date, lat, lon, time)
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.connect((server_ip, server_port))
                message = {
                    "command": "point_to_constellation",
                    "ra": ra,
                    "dec": dec,
                    "date": date,
                    "lat": lat,
                ...
                print(f'Received response from server: Photo of size {photo_size} bytes')
                
                # Actualitzar la interfície d'usuari amb la foto
                if update_image:
    ...
    def send_request_for_photo(server_ip='192.168.1.128', server_port=65432, update_image=None):
        def send_request():
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.connect((server_ip, server_port))
    ...
    def process_image(input_img):
        ...
        if best_match_name:
            ...
            return None, "No se encontró ninguna coincidencia adecuada."
